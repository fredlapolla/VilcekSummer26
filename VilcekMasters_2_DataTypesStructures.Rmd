...
```{r}
nyc <- read.csv("https://raw.githubusercontent.com/fredlapolla/VilcekSummer26/main/NYC_HANES_DIAB.csv")
```
...
Date formatted data is essentially what it sounds like, a field such as 5/13/2026 that can be interpreted mathematically to tell you how long apart different events occurred.

Often R will default to read in date data as character, and a useful command is as.Date()

```{r}
nyc$BIRTHDATE_SIMULATED <- as.Date(nyc$BIRTHDATE_SIMULATED, format = "%m-%d-%Y")
```

Additional useful tools are the lubridate package, which includes options like floor_date() to round years down, which can be helpful for ages. 
...
When adding labels, which will be text values, it is important that they match the order of the values they are replacing. So in this data, we had 1, 2 and 3 representing youngest, middle and aged groups, and so the order of the text needs to match that. 

It is helpful to use labels to avoid confusion later when making cross-tabulations. 

Unlike in SPSS and some other tools, the labels replace the original values completely in R.


```{r}
nyc$AGEGROUP <- factor(nyc$AGEGROUP, labels = c("Youth", "Middle", "Aged"))
